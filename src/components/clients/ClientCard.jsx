...
const ClientCard = ({ id, name, email, company, initials, colorClass = 'bg-indigo-500/30 border-indigo-400/20' }) => {
  ...
  return (
    <div className="group relative glass-card rounded-[20px] transition-all duration-500 hover:bg-white/[0.08] hover:-translate-y-2 overflow-hidden shadow-lg shadow-black/20">
      <Link href={`/clients/${id}`} className="block p-8">
        {/* Card Header: Avatar and Actions */}
        <div className="flex items-start justify-between">
          {/* Client Avatar / Initials */}
          <div className={`w-14 h-14 rounded-full flex items-center justify-center font-bold text-lg text-white border ${colorClass}`}>
            {initials}
          </div>
          ...
          <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 relative z-10" onClick={(e) => e.preventDefault()}>
            ...
            <button
              type="button"
              className="w-10 h-10 rounded-full glass-card flex items-center justify-center text-on-surface-variant hover:text-error transition-colors"
              title="Delete Client"
        ...
        {/* Client Info Section */}
        <div className="mt-8">
          <h3 className="font-body text-xl font-semibold text-white">{name}</h3>
      ...
      <div className="absolute -bottom-10 -right-10 w-24 h-24 bg-indigo-500/10 blur-[40px] rounded-full group-hover:bg-indigo-500/20 transition-colors pointer-events-none" />
    </div>
  );
};
